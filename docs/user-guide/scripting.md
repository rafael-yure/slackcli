...
`messages draft` (create, send, delete), the `usergroups` write verbs (`create`, `update`, `add`,
...
```bash
sent=$(slackcli messages draft --send="$DRAFT_ID" --workspace=rafael --yes --json)
ts=$(jq -r '.ts' <<<"$sent")
```

`--send` removes the draft after posting. If that cleanup fails, the command
...
`messages draft --delete="$DRAFT_ID" --yes --json` to discard an unneeded draft.
