...
### Added
- **Draft lifecycle**: `messages draft --send <draft-id>` posts a reviewed text draft to its saved channel/thread and removes it after delivery; `--delete <draft-id>` discards it without posting. Both require browser auth and accept `--yes` for unattended use (#272)
